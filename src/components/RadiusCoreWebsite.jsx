...
import { useReveal, Logo, Tag, ArrowRight, PrimaryBtn, SectionLabel, NAV_LINKS, SiteFooter } from './siteShared.jsx';
...
export default function RadiusCoreWebsite() {
  ...
  // When arriving from another page via /#section, scroll to that section.
  useEffect(() => {
    if (!hash) return;
    const id = hash.slice(1);
    // Wait a tick so the section is mounted and the route-level ScrollToTop
    // (which resets to top on the /careers → / transition) has already run.
    const t = setTimeout(() => {
